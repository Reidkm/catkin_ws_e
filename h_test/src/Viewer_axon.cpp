...
//std::stringstream ss;
cv::VideoWriter outputVideo_depth;
cv::VideoWriter outputVideo_normal;

cv::String outputvideopath_depht = "/home/reid/catkin_ws/depth.avi";
cv::String outputvideopath_normal = "/home/reid/catkin_ws/normal.avi";
int putvideoindex = 1;
	...
	for (int j = 510; j < 530; j++)
	{
		...
		*/
		...
		// for plastic pallet
			...
			//if(ptr[i+7]-ptr[i] > 200 && right_pixel_leg == 1 && ptr[i]> min_depth_data && ptr[i+7]> min_depth_data)
			//{
				//std::cout << "i is " << i  << std::endl;
			//	right_pixel_leg = i+5 ;
        	//    break;
		...
		//if( left_pixel_leg != 1 && right_pixel_leg ！= 1)
		//{
		//	break;
		//}
	std::fstream outfile;
	...
	for(int i = 0; i < m_width; i++)
	{
		...
		for(int j = 0;j <mid_pixel_leg ;j++ )
		{
			...
			if((ptr[j]-ptr[j+2])> 500 && ptr[j] > 2000 && ptr[j+2] > 2000 && ptr[j] < max_depth_data && ptr[j+2] < max_depth_data   )
			{
				...
				if(check_border_count > 97 && j+4 < goods_border_column)
				{
					
					goods_border_column = j+4 ;
					goods_border_row= i;
...
{	
	...
	int temp_count=0;
	//std::vector<int> all_pixel_count;
	...
	{	
		...
		{
			const ushort *ptr = img.ptr<ushort>(j);
			if(ptr[i] > 0)
			{
				pixel_count++;
			}
		...
		//all_pixel_count.push_back(pixel_count);
	}
	//goods_border_row = 1;
	...
	{	
		const ushort *ptr = img.ptr<ushort>(i);
		if(ptr[goods_border_column+2] > 0)
		{
			goods_border_row = i;
			break;
		}
...
void SampleViewer::Histogram()
{
	...
	for(int i = 0;i < 350; i++)
	{
		...
		//std::cout << "ptr515_" << i << " is " << ptr_515[i] << std::endl;
	}
...
openni::Status SampleViewer::init(int argc, char **argv)
{
	...
	//outputVideo_depth.open(outputvideopath_depht,CV_FOURCC('M','P','4','2'),25.0,cv::Size(640,480));
	//outputVideo_normal.open(outputvideopath_normal,CV_FOURCC('M','P','4','2'),25.0,cv::Size(480,640));
...
void SampleViewer::display()
{
	...
	if(m_depthFrame.isValid())
	{
		...
		int oversize_pixel = CalculateOverSizePixel(ptr_1[goods_border_column+5],ptr_2[mid_pixel_leg]);
		...
		//line(image_normal, cv::Point(right_pixel_leg, 0), cv::Point(right_pixel_leg, m_width), cv::Scalar(0), 1, 8);
		...
		if (putvideoindex ==1)
		{
			outputVideo_normal << image_normal ;
			outputVideo_depth << src;
		}
		if (putvideoindex == 0)
		{
			outputVideo_normal.release();
		...
		for(int i = 0; i < m_width; i++)
		{
			...
			for(int j = 0;j <mid_pixel_leg ;j++ )
			{
				...
				//if()
				{
					...
					//cout << "i =" << i << "," << "j = " << j << endl; 
					//goods_border_column = std::min(j+3,goods_border_column);
					for (int  i = 0; i < 100; i++)
					{	
					...
					//if(j+10 < goods_border_column)
					//{
					//	goods_border_column = j+10 ;
					//	goods_border_row= i;
					//	
					//}
					//break;
    ...
    /*
		for (int y = 0; y < divide_row; y++){
			for (int x = 0; x < mid_pixel_leg; x ++){		
            	...
            	{
					goods_border_column = std::min(x,goods_border_column);	
					break;
				}
			}	  
		}*/
