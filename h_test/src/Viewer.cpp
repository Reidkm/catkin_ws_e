...
openni::Status SampleViewer::init(int argc, char **argv)
{
	...
	if (m_depthStream.isValid() && m_colorStream.isValid())
	{
		...
		std::cout << "depthWidth is "  << depthWidth  <<"\n"
			<< "depthHeight is "  << depthHeight  <<"\n"  
			<< "colorWidth is "  << colorWidth  <<"\n" 
...
void SampleViewer::onKey(unsigned char key, int /*x*/, int /*y*/)
{
	...
	case '1':
		...
		//std::cout << "openni::Status rc is " << rc << std::endl;
		break;
		}
	case '2':
	...
	case 'w':
		...
		ss << img_count;
		ss >> temp_str;
		cv::Mat img_depth(m_depthFrame.getHeight(), m_depthFrame.getWidth(), CV_16UC1, (void*)m_depthFrame.getData());
		cv::Mat img_rgb(m_colorFrame.getHeight(), m_colorFrame.getWidth(), CV_8UC3,(void*)m_colorFrame.getData());
		//cv::Mat im_RGB_1(960, 1280, CV_8UC3,(void*)m_colorFrame.getData());
		//cv::Mat m_colorFrame = cv::Mat(m_depthFrame.getHeight(), m_depthFrame.getWidth(), CV_8UC3, (void*)m_colorFrame.getData());
		cv::Mat converted;
		cv::cvtColor(img_rgb,converted,cv::COLOR_RGB2BGR);
		cv::imwrite("img_depth_"+temp_str+"_L30.png", img_depth);
		cv::imwrite("img_rgb_"+temp_str+"_L30.png", converted);
		//cv::imwrite("im_RGB_1_forjerry.png", im_RGB_1);
		//cv::imwrite("m_colorFrame.png", m_colorFrame);
		img_count = img_count + 5;
		break;
