...
int main() 
{ 
	ofstream fout("/home/reid/Desktop/tem_folder/webcamera_undistort/temp_img_dsipc/caliberation_result.txt"); /**    保存定标结果的文件     **/ 
	//cv::FileStorage fs("/home/reid/Desktop/tem_folder/webcamera_undistort/temp_img_dsipc/test.yaml", cv::FileStorage::WRITE); 
...
/************************************************************************
读取每一幅图像，从中提取出角点，然后对角点进行亚像素精
*************************************/
	cout << "开始提取角点………………" << endl; 
	int image_count = 23; /****    图像数量     ****/ 
	Size board_size = Size(7, 4); /****    定标板上每行、列的角点数       ****/
	...
	{ 
		...
		ss << i + 2; 
		ss >> str;
		strFilename = "/home/reid/Desktop/tem_folder/webcamera_undistort/temp_img_dsipc/img_"+str+".jpg";
		Mat image = imread(strFilename); /* 提取角点 */ 
		Mat imageGray; 
		...
		if (!patternfound) 
		{ 
			cout << "找不到角点，需删除图片文件" << strFilename << "重新排列文件名，再次标定" << endl; 
			getchar(); 
			exit(1); 
		} 
		else 
		{ 
			...
			{ 
				...
				//drawChessboardCorners(imageTemp, cv::Size(4, 7), corners[j], true);
				drawChessboardCorners(imageTemp,board_size,corners,true);
			} 
			...
			imageFileName = "/home/reid/Desktop/tem_folder/webcamera_undistort/temp_img_dsipc/img_"+imageFileName+"_corner.jpg";
...
*************************************************************************/ 
	cout << "开始定标………………" << endl; 
	Size square_size = Size(70, 70); 
	vector<vector<Point3f> > object_Points;              /****  保存定标板上角点的三维坐标   ****/ 
	Mat image_points = Mat(1, count, CV_32FC2, Scalar::all(0));               /*****   保存提取的所有角点   *****/ 
	vector<int> point_counts;             /* 初始化定标板上角点的三维坐标 */ 
	...
	Mat intrinsic_matrix; /*****    摄像机内参数矩阵    ****/ 
	Mat distortion_coeffs; /* 摄像机的4个畸变系数：k1,k2,k3,k4*/ 
	...
	fwrite<< "mtx"<<intrinsic_matrix ;
	fwrite<< "dist"<<distortion_coeffs ;
	fwrite.release(); 
/************************************************************************
对定标结果进行评价
*************************************************************************/ 
	...
	for (int i = 0; i<image_count; i++) 
	{ 
		vector<Point3f> tempPointSet = object_Points[i]; //    通过得到的摄像机内外参数，对空间的三维点进行重新投影计算，得到新的投影点     
...
/************************************************************************
    ...
    *************************************************************************/ 
	...
	Mat img_23 = imread("/home/reid/Desktop/tem_folder/webcamera_undistort/temp_img_dsipc/img_23.jpg"); 
	cv::remap(img_23, t, mapx, mapy, INTER_LINEAR);
	imshow("undistort",t);
	waitKey(0);
/*
