...
struct OniStreamServices
{
	...
	int (ONI_CALLBACK_TYPE* checkFrameAllocatedSize)(OniFrame* pFrame, const int size, int* pAllocatedSize); //return TRUE if allocated size for pFrame is >= size. return allocated size in pAllocatedSize if it is not NULL.
};
